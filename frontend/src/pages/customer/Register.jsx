...
export default function CustomerRegister() {
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      await customerService.register(data.name, data.email, data.password);
      navigate('/login');
    } catch (err) {
  ...
  return (
    <div className="min-h-[70vh] flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center space-y-3">
        ...
        <p className="text-sm text-gray-500 font-semibold">Join us to support local farmers & order fresh harvests</p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 border border-gray-200 shadow-md sm:rounded-3xl sm:px-10 space-y-6">
          ...
          <div className="text-center pt-2">
            <span className="text-sm text-gray-400 font-medium">Already have an account? </span>
            <Link to="/login" className="text-sm font-bold text-emerald-600 hover:text-emerald-700 transition">
              Sign In
            </Link>
          </div>
        </div>
